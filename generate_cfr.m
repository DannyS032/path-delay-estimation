% CFR Generation Function - Path Delay Estimation project
% Written by Danny Sinder
function [r, r_h, L, lambda, tau_0 ,tau_l, E_hl_2, sigma_l_2, h, F, sigma_w_2, w, snr] = ...
    generate_cfr(Fs, N, pilot_index, tau_rms, snr_case, up_sample)
    % Simulates the CFR signal based on input parameters
    % Inputs:
    ...
    % Step 1: Uniformly pick L and λ
    [L, lambda] = pick_L_and_lambda();
    ...
    % Step 8: Uniformly pick SNR for relevant case
    snr = pick_snr(snr_case);

    % Step 9: Calculate noise frequency variance σ_w^2
    sigma_w_2 = calculate_noise_variance(snr, N*up_sample);

    % Step 10 & 11: Generate noise vector w
    w = generate_noise_vector(sigma_w_2, num_pilots);

    % Step 12: Generate High-res. noiseless CFR signal
    r_h = F * h;

    % Step 13: Generate zero-padded Low-res. noisy CFR signal
...
%%%%%%%%%%% Helper-functions %%%%%%%%%%%
function [L, lambda] = pick_L_and_lambda()
    L = randi([3 15]); % Number of taps (random within [3, 15])
...
function snr = pick_snr(snr_case)
    if snr_case == 'h' 
        snr = (10 + (20)*rand); % SNR for high case (random within [10, 30]dB)
    elseif snr_case == 'l' 
        snr = (10*rand); % SNR for low case (random within [0, 10]dB)
    end
end
